...
--rate
      local lim_rate, err_rate = limit_req.new(limit_store, tonumber(rule.attack_ip_qps), tonumber(rule.attack_ip_expire_qps))
        if not lim_rate then
          ...
          error_info['log_type'] = "error_log"
          error_info['error_type'] = "limit_req"
          error_info['error_info'] = "limit_req_domain_rate,failed to instantiate a resty.limit.count object: "..err_rate_attack.." limit_store is: "..limit_count_store
          ngx.ctx.error_log = error_info
          ngx.log(ngx.ERR,"limit_req_domain_rate,failed to instantiate a resty.limit.req object: ", err_rate_attack," limit_store is: ",limit_store)
          return  ngx.exit(500)
        end
