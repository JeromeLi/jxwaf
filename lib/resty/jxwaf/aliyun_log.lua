...
aliyun_log.log_producer_config_set_packet_log_bytes(producer_config, 4*1024*1024);
aliyun_log.log_producer_config_set_packet_log_count(producer_config, 4096);
...
aliyun_log.log_producer_config_set_send_timeout_sec(producer_config, 15);
...
aliyun_log.log_producer_config_set_net_interface(cproducer_config, nil);
config = aliyun_log.create_log_producer(producer_config,nil)
if not config then
	ngx.log(ngx.ERR,"create log producer by config fail")
end
client = aliyun_log.get_log_producer_client(config,nil)
if not client then
...
local result =aliyun_log.log_producer_client_add_log(client,2,"eeeee","eeeee")
